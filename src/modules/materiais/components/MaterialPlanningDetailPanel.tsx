...
export function MaterialPlanningDetailPanel({
  ...
  const getStatusBadge = (status: MaterialStockStatus) => {
    switch (status) {
      case "adequado":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-[11px] font-mono uppercase tracking-wider bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 font-medium">
      ...
      case "atencao":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-[11px] font-mono uppercase tracking-wider bg-amber-500/15 text-amber-400 border border-amber-500/30 font-medium">
      ...
      case "critico":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-[11px] font-mono uppercase tracking-wider bg-rose-500/15 text-rose-400 border border-rose-500/30 font-medium">
    ...
    <div className="bg-[#0c1524] border border-blue-500/20 rounded-lg p-5 space-y-4 shadow-xl text-slate-200">
      ...
      <div className="bg-[#070c14] border border-blue-500/15 rounded-md p-3.5 space-y-2 text-xs">
        ...
        <div className="flex justify-between items-center py-1 bg-[#131f33] px-2 rounded font-bold">
          <span className="text-white font-mono">Estoque Final Projetado:</span>
      ...
      <PermissionGate permission="estoque.movimentar">
        <div>
          <button
            type="button"
            onClick={onAddStock}
            className="w-full text-xs font-bold py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-md shadow-[0_0_15px_-3px_rgba(249,115,22,0.4)] transition-all active:scale-[0.99]"
          >
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
            {linkedActivities.map((act) => (
              ...
              >
                ...
                <p className="font-semibold text-white truncate">{act.activityName}</p>
