...
export default function PinturaInicioPage() {
  ...
  return (
    <div className="space-y-6">
      {/* 1. CABEÇALHO DA CENTRAL OPERACIONAL (Dark Premium Industrial) */}
      <div className="bg-[#0f172a] border border-white/10 rounded-lg p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-md">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/pintura/programacao"
            className="text-xs font-bold px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded transition-colors shadow-[0_0_12px_-2px_rgba(16,185,129,0.3)]"
          >
            Abrir Programação →
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-[#0f172a] border border-white/10 rounded-lg p-3.5 flex flex-col justify-between">
          ...
          <span className="text-[10px] text-slate-500">em andamento / programadas</span>
        </div>
        ...
        <div className="bg-[#0f172a] border border-white/10 rounded-lg p-3.5 flex flex-col justify-between">
          ...
          <span className="text-[10px] text-slate-500">avanço médio de todas as OSs</span>
        </div>
        ...
        <div className="bg-[#0f172a] border border-white/10 rounded-lg p-3.5 flex flex-col justify-between">
          ...
          <span className="text-[10px] text-slate-500">de {stockSummary.total} materiais cadastrados</span>
        </div>
      </div>
          ...
          {weekDaysActivities.map((day) => (
            <Link
              ...
              className={`p-3 rounded-lg border transition-all flex flex-col justify-between ${
                day.isToday
                  ? "bg-[#090d16] border-emerald-500/50 shadow-[0_0_12px_-2px_rgba(16,185,129,0.3)] ring-1 ring-emerald-500/30"
                  : "bg-[#090d16] border-white/10 hover:border-white/20 hover:bg-white/5"
              }`}
            >
              <div className="flex justify-between items-center">
                <span className={`text-xs font-bold ${day.isToday ? "text-emerald-400 font-mono" : "text-slate-300"}`}>
                  {day.dayOfWeek}
                </span>
                <span className="text-[10px] font-mono text-slate-400">{day.label}</span>
              </div>
              <div className="mt-3">
                <span className="text-xl font-bold font-mono text-slate-100">{day.count}</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {shortcuts.map((s) => (
            <Link
              key={s.href}
              href={s.href}
              className="p-3.5 bg-[#090d16] border border-white/10 rounded-lg hover:border-emerald-500/40 hover:bg-white/5 transition-all block group"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-100 group-hover:text-emerald-400 transition-colors block truncate">
                  {s.title}
                </span>
                <span className="text-[10px] text-slate-500 font-mono group-hover:text-emerald-400">→</span>
              </div>
